...
public class tests{
    public static void main(String args[]){
        ArrayList<Cell> board = makeBoard();
        board=AC3(board);
        System.out.println();
        for(int i=0; i<81; i++){
            //if it's not, run the backtracking alg
            if (board.get(i).domain.size()!=1){
                System.out.println("About to run backtrack for index: " + i + " with domain: " + board.get(i).domain.size());
    ...
    public static Boolean Revise(ArrayList<Cell> board, Cell a, Cell b){
        ...
        for (int x=0; x<a.domain.size(); x++){
            ...
            if (satisfied){
                if(board.get(0).equals(a)){
  //                 System.out.println("Removing domain from cell 0 because of cell: " + board.indexOf(b));
    //                System.out.println("Domain of cell 20: " + b.domain.size());
                    for(int poss : b.domain){
    ...
    public static ArrayList<Cell> makeBoard(){
        ArrayList<Cell> board=new ArrayList<Cell>();
        board.add(new Cell(2));
        for (int i=0; i<3; i++){
            board.add(new Cell());
        }
        board.add(new Cell(8));
        board.add(new Cell());
        board.add(new Cell(3));
        for (int i=0; i<3; i++){
            board.add(new Cell());
        }
        board.add(new Cell(6));
        board.add(new Cell());
        board.add(new Cell());
        board.add(new Cell(7));
        board.add(new Cell());
        board.add(new Cell());
        board.add(new Cell(8));
        board.add(new Cell(4));
        board.add(new Cell());
        board.add(new Cell(3));
        board.add(new Cell());
        board.add(new Cell(5));
        board.add(new Cell());
        board.add(new Cell());
        board.add(new Cell(2));
        board.add(new Cell());
    ...
    public static void printer(ArrayList<Cell> board){
         //print out the completed board
         for (int i=0; i<81; i++){
            if(i%9==0){
                System.out.println();
